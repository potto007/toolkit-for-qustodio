...
Qustodio has the ability to set rules for apps. These rules are at the _individual-level_, which
makes it time-consuming to disable and re-enable groups of apps (ie: games) when the need arises.
It should be possible to create "app groups" and set rules by group. Better still: set time limits
_by app group_. Rather than ask the Qustodio team to implement these features, let's just do it
ourselves!
...
- Chrome on the [Chrome Web Store](https://chrome.google.com/webstore/detail/toolkit-for-Qustodio/lmhdkkhepllpnondndgpgclfjnlofgjl)
- Firefox on the [Firefox Add-on Repository](https://addons.mozilla.org/firefox/addon/toolkit-for-Qustodio/)
...
1. Add / remove apps to default group
1. Disable / enable default group's apps
1. Add group management popup
1. Add group management functionality
1. Add / remove apps to user-defined groups
1. Disable / enable user-defined groups' apps
...
directories which represent each section of the Qustodio application. You should select
...
**IMPORTANT NOTE:** This extension is not affiliated with Qustodio in any way and Qustodio
has not endorsed this at all.
