...
const filesToCopy = glob.sync(path.join(workspaceRoot, 'dist/toolkit-for-qustodio-v*.zip'));

if (filesToCopy.length !== 1) {
  throw new Error(`There must be precisely one zip file to copy. Found ${filesToCopy.length}!`);
}

fs.copyFileSync(filesToCopy[0], path.join(desktopUpdatesPath, 'toolkitforqustodio_desktop.zip'));
